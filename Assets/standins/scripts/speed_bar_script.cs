...
public class speed_bar_script : MonoBehaviour 
{
	RectTransform location;
	GameObject player;
	move_player_script player_script;
	float speed;
	float y_max;
	float y_min;
	float x;
	...
	void Start () 
	{
		...
		player_script = (move_player_script)player.GetComponent (typeof(move_player_script));
